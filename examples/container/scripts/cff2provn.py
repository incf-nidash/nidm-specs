...
cml_collection.add_extra_attributes({prov.PROV['type']: cml['ConnectomeFileFormat'], prov.PROV['type']: cml['CMLTagCollection'],prov.PROV['label']:filename})
...
cml_meta.add_extra_attributes({prov.PROV['type']:cml['ConnectomeMeta'],cml['species']:species,cml['timepoint']:tp,cml['subject_name']:sub})
...
for v in volumes:
  ...
  cml_volume.add_extra_attributes({prov.PROV['type']:cml['connectome-volume'],cml['dtype']:dtype,cml['src']:src,cml['name']:name,cml['fileformat']:fileformat})
...
for t in tracks:
  ...
  cml_track.add_extra_attributes({prov.PROV['type']:cml['connectome-track'],cml['dtype']:dtype,cml['src']:src,cml['name']:name,cml['fileformat']:fileformat})
...
for n in networks:
  ...
  cml_network.add_extra_attributes({prov.PROV['type']:cml['connectome-network'],cml['dtype']:dtype,cml['src']:src,cml['name']:name,cml['fileformat']:fileformat})
...
for s in surfaces:
  ...
  cml_surface.add_extra_attributes({prov.PROV['type']:cml['connectome-surface'],cml['dtype']:dtype,cml['src']:src,cml['name']:name,cml['fileformat']:fileformat})
...
for d in data:
  ...
  cml_data.add_extra_attributes({prov.PROV['type']:cml['connectome-data'],cml['dtype']:dtype,cml['src']:src,cml['name']:name,cml['fileformat']:fileformat})
